...
import createHalfDuplexChatAdapter, {
  ...
  type TurnGenerator
} from '../../createHalfDuplexChatAdapterWithSubscribe';
  ...
  describe.each([true, false])('With emitStartConversationEvent of %s', emitStartConversationEvent => {
    describe.each([
      ...
      beforeEach(() => {
        ...
        httpPostConversation = jest.fn(throwOnCall<DefaultHttpResponseResolver>('httpPostConversation is not mocked'));
...
`),
            ...
            httpPostSubscribe.mockImplementationOnce(
              () =>
                new HttpResponse(mockHTTPPostSubscribeReadableStream().readableStream, {
          ...
          describe('should POST to /subscribe', () => {
            ...
            test('with header "Content-Type" of "application/json"', () =>
              expect(httpPostSubscribe.mock.calls[0][0].request.headers.get('content-type')).toBe('application/json'));
            ...
            test('with header "x-ms-conversationid" of "c-00001"', () =>
              expect(httpPostSubscribe.mock.calls[0][0].request.headers.get('x-ms-conversationid')).toBe('c-00001'));

            if (shouldSetCorrelationId) {
              test('with header "x-ms-correlation-id" of "t-00001"', () =>
                expect(httpPostSubscribe.mock.calls[0][0].request.headers.get('x-ms-correlation-id')).toBe('t-00001'));
            } else {
...
`)
            ...
            test('should not POST to /conversations/c-0001', () => expect(httpPostExecute).toHaveBeenCalledTimes(0));
...
`)
              ...
              ])('when %s', (_, outgoingActivity) => {
                let generator: TurnGenerator;

                beforeEach(() => {
                  shouldSetCorrelationId && getCorrelationId.mockReset().mockImplementation(() => 't-00002');
                  generator = (iteratorResult.value as ExecuteTurnFunction)(outgoingActivity);
                });
